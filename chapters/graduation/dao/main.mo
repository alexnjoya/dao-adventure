...
actor {
        ...
        public shared ({ caller }) func registerMember(name : Text) : async Result<(), Text> {
                switch(members.get(caller)) {
                        ...
                        case (?member) {
                                #err("Caller already part of the DAO");
                        };
                }
        };
        ...
        public shared ({ caller }) func graduate(student : Principal) : async Result<(), Text> {
                switch(members.get(caller)) {
                        case(null) {
                                return #err("Caller is not an authorized member of DAO");
                        };
                        case(?mentor) {
                                ...
                                switch(members.get(student)) {
                                        ...
                                        case (?member) {
                                                if (member.role != #Student) {
                                                        return #err("The member is not currently a student");
                                                };
                ...
                switch (members.get(caller)) {
                        ...
                        case (?member) {
                                if(member.role != #Mentor) {
                                        return #err("Only mentor can make proposal");
                                };
                                ...
                                switch (content) {
                                        case(#AddMentor(newMentor)) {
                                                switch(members.get(newMentor)) {
                                                        case(null) { return #err("Mentor candidate isn't part of the DAO")};
                                                        case(?tobeMentor) {
                                                                if (tobeMentor.role != #Graduate) {
                                                                        return #err("Only Graduates can become mentor");
                                                                };
                                                        }
                                                }
                                        };
        ...
        public shared ({ caller }) func voteProposal(proposalId : ProposalId, yesOrNo : Bool) : async Result<(), Text> {
                switch (members.get(caller)) {
                        ...
                        case (?member) {
                                if (member.role == #Student) {
                                        return #err("Students aren't allowed to vote");
                                };
